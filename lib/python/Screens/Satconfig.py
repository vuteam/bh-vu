...
class NimSetup(Screen, ConfigListScreen, ServiceStopScreen):
    ...
    def createConfigMode(self):
        if self.nim.isCompatible('DVB-S'):
            ...
            if self.nim.isFBCLink():
                choices = {'nothing': _('FBC automatic loop through'),
                 'advanced': _('FBC Unicable')}
    ...
    def createSetup(self):
        ...
        if self.nim.isCompatible('DVB-S'):
            ...
            if self.nim.description == 'Alps BSBE2' and config.usage.setup_level.index >= 2:
    ...
    def __init__(self, session, slotid):
        ...
        self['key_blue'] = Label()
        self['actions'] = ActionMap(['SetupActions', 'SatlistShortcutAction'], {'ok': self.keyOk,
         'save': self.keySave,
         'cancel': self.keyCancel,
         'changetype': self.changeConfigurationMode,
         'nothingconnected': self.nothingConnectedShortcut}, -2)
        self.slotid = slotid
...
class NimSelection(Screen):
    ...
    def __init__(self, session):
        ...
        self.updateList()
        self.setResultClass()
        self['actions'] = ActionMap(['OkCancelActions', 'MenuActions', 'ChannelSelectEPGActions'], {'ok': self.okbuttonClick,
         'info': self.extraInfo,
         'epg': self.extraInfo,
         'cancel': self.close,
         'menu': self.exit}, -2)
